          ...
          {berylliumSidebarMenuItems.map((item, index) => {
            ...
            return (
              <Fragment key={item.name + '-' + index}>
                {/* Swap static Pre-Sales Dashboard with dynamic workspace group */}
                {item.name === 'Pre-Sales Dashboard' ? (
                  <div className="mx-3 my-0.5 lg:my-1 2xl:mx-5 2xl:my-2">
                    <PresalesSidebarGroup />
                  </div>
                ) : item?.href ? (
                  <>
                    {item?.dropdownItems ? (
